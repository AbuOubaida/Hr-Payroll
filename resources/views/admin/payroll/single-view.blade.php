...
<div class="row">
    <div class="col-12">
        ...
        @if(!$salary->paid_status)
        <form action="{{route('make.salary.paid')}}" class="d-inline-block float-right" method="post">
            <input type="hidden" name="salary" value="{{$salary->sa_id}}">
            <button type="submit" class="btn btn-outline-success" onclick="return confirm('Are you sure, to make this salary paid?')">Make Salary Paid</button>
        </form>
        @endif
    </div>
</div>
