@if(count($projects))
    ...
    @foreach($projects as $p)
        <tr class="@if($p->project_id == \Illuminate\Support\Facades\Request::route('pEditID') || $p->project_id == \Illuminate\Support\Facades\Request::route('pShowID')){{'text-white'}}@endif">
            <td>{{$i++}}</td>
            <td>{{$p->p_title}} @if($p->seen_status == 0)<sup class="text-danger">New</sup> @endif</td>
            <td>
                {{$p->p_duration}}
            ...
            <td>{{$p->p_end_date}}</td>
            <td>{{$p->team_title}}</td>
            <td>{{\Illuminate\Support\Facades\Auth::user()->name}} (Me)</td>
            ...
            <td class="text-center">
                ...
                <a href="{{url('project-manager/project/view-project/'.$p->set_teams_id)}}" class="text-warning" title="Full view of project"> <i class="mdi mdi-eye"></i></a>
            </td>
        </tr>
    @endforeach
    @else
